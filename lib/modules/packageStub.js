...
const packageStub = async function ({ PATHS } = {}) {
...
const stubThemeSources = function ({ PATHS }) {
  ...
  const themeSourcesFiles = [
    ...
    path.join(PATHS.theme, 'src', 'theme.scss'),
    path.join(PATHS.theme, 'src', 'theme.ts')
  ];

  themeSourcesFiles.forEach((file) => {
    const localPath = path.relative(PATHS.root, file);

    // Path without the file name is required to not create folder with the file name
    const destinationFilePath = localPath.split(path.sep).slice(0, -1).join(path.sep);
...
const stubTheme = async function ({ PATHS }) {
  ...
  files.forEach((file) => {
    if (path.dirname(file) === '.') {
      ...
      zipTheme.addLocalFile(path.join(tempDistPath, file), 'theme-sources');
    }
  });
